...
@app.get("/users/", response_model=List[schemas.OnlyUser])
...
def update_user(user_id: int, user: schemas.UserUpdate, db: Session = Depends(get_db)):
    response = crud.update_user(user_id=user_id, user=user, db=db)
    return response


@app.delete("/users/{user_id}")
def delete_user(user_id: int, db: Session = Depends(get_db)):
    response = crud.delete_user(user_id=user_id, db=db)
    return response
